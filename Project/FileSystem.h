...
extern int disk_empty;	 //连续存储的磁盘大小
...
public:
	...
	int findFile(MyDir *dir, char *name);	//查找文件
	int findFiles();						//查找相同后缀文件

	int   exportFile(string &text);			//导出文件到指定位置
	char* importFile();						//导入文件到指定目录下的文件中
	...
	char *getRootName();					//获取根目录名称

	int getVersion();						//获取文件系统版本信息
};
